...
namespace Sahll.JUI.Car
{
    public partial class CarsProductsFRM : Form
    {
            ...
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvcarproduct.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                switch (columnName)
                {
                    ...
                    case "edit":
                        ...
                        frm.lblCarsAddEditProductsEnventoryFRM.Text = "تعديل صنف عند السياره";
                        frm.Show();
                        break;
                    case "returnd":
        ...
        {
            var title = $"تقرير بالبضاعه المورده للسياره ليوم {DateTime.Now:dddd} بتاريخ {DateTime.Now:yyyy-MM-dd}";
            ...
            string salesReportsFolder = Path.Combine(cars, "البضاعه المورده");
            ...
            {
                var worksheet = workbook.Worksheets.Add("بضاعة السيارات");
                ...
                titleCell.Style.Font.FontSize = 9;
        ...
        private void dgvcarproduct_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
