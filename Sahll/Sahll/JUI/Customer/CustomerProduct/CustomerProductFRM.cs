...
namespace Sahll.JUI.Selling
{
    public partial class CustomerProductFRM : Form
    {
        ...
        {
            InitializeComponent();
            dgvCustomerProduct.Rows.Add(new object[] { "صابون سائل", 527, 2156, 2156, "الخميس" });
            dgvCustomerProduct.Rows.Add(new object[] { " اريال", 524, 2156, 2156, "الخميس" });
            dgvCustomerProduct.Rows.Add(new object[] { "زيت دابر املا 100 عادي", 524, 2156, 2156, "الخميس" });
            dgvCustomerProduct.Rows.Add(new object[] { " دابر املا 100 دهبي", 524, 2156, 2156, "الخميس" });
        }
        ...
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvCustomerProduct.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                string columnName = dgvCustomerProduct.Columns[e.ColumnIndex].Name;
                ...
                switch (columnName)
                {
                    ...
                    case "edit":
                        ...
                        CustomerProductEditFRM FRM = new CustomerProductEditFRM();
        ...
        private void btnreport_Click(object sender, EventArgs e)
        {
            ...
            ReportToExcel(dgvCustomerProduct);
        }
        ...
        private void dgvSelling_CellMouseEnter(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvCustomerProduct.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                {
                    case "delete":
                        dgvCustomerProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.deleted1;
                        break;

                    case "edit":
                        dgvCustomerProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.editd;
                        break;
                    case "returnd":
                        dgvCustomerProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.returnedd;
                        break;

                    case "info":
                        dgvCustomerProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.infod;
                        break;
                }
            }
        }
        ...
        private void dgvSelling_CellMouseLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvCustomerProduct.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                {
                    case "delete":
                        dgvCustomerProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.deletel1;
                        break;

                    case "edit":
                        dgvCustomerProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.editl;
                        break;
                    case "returnd":
                        dgvCustomerProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.returnedl;
                        break;

                    case "info":
                        dgvCustomerProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.infol;
                        break;
                }
            }
        }
        ...
        {
            var title = $"تقرير بضاعة للعميل ليوم {DateTime.Now:dddd} بتاريخ {DateTime.Now:yyyy-MM-dd}";

            // تحديد مسار مجلد "تقارير سهل" داخل "سطح المكتب"
            string desktopPath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
            string easyReportsFolder = Path.Combine(desktopPath, "تقارير سهل");
            string salesReportsFolder = Path.Combine(easyReportsFolder, "العملاء(بضاعه)");
            ...
            {
                var worksheet = workbook.Worksheets.Add("المقبوضات");
                ...
                companyCell.Style.Font.FontSize = 9;
