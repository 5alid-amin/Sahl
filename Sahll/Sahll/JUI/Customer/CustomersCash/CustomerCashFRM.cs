...
namespace Sahll.JUI.Selling
{
    public partial class CustomerCashFRM : Form
    {
        ...
        {
            InitializeComponent();
            dgvCustomerCash.Rows.Add(new object[] { 52327, "هذا الشخص من المحتمل ان لا يرجع النقود", "الخميس", "28/12/2004", "عصام" });
            dgvCustomerCash.Rows.Add(new object[] { 52327, "هذا الشخص من المحتمل ان لا يرجع النقود", "الخميس", "28/12/2004", "عصام" });
            dgvCustomerCash.Rows.Add(new object[] { 52327, "هذا الشخص من المحتمل ان لا يرجع النقود", "الخميس", "28/12/2004", "عصام" });
        ...
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvCustomerCash.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                string columnName = dgvCustomerCash.Columns[e.ColumnIndex].Name;

                switch (columnName)
                {
                    case "edit":
                        CustomerCashAddEditFRM FRM = new CustomerCashAddEditFRM();
                        FRM.lblCustomerCashAddEditFRM.Text = "تعديل نقدية عميل";
        ...
        private void dgvSelling_CellMouseEnter(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvCustomerCash.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                {
                    case "delete":
                        dgvCustomerCash.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.deleted1;
                        break;

                    case "edit":
                        dgvCustomerCash.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.editd;
                        break;

                    case "info":
                        dgvCustomerCash.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.infod;
                        break;
                }
            }
        }
        ...
        private void dgvSelling_CellMouseLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvCustomerCash.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                {
                    case "delete":
                        dgvCustomerCash.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.deletel1;
                        break;

                    case "edit":
                        dgvCustomerCash.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.editl;
                        break;

                    case "info":
                        dgvCustomerCash.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.infol;
                        break;
                }
            }
        }
        ...
        {
            var title = $"تقرير نقدية العميل ليوم {DateTime.Now:dddd} بتاريخ {DateTime.Now:yyyy-MM-dd}";

            // تحديد مسار مجلد "تقارير سهل" داخل "سطح المكتب"
            string desktopPath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
            string easyReportsFolder = Path.Combine(desktopPath, "تقارير سهل");
            string salesReportsFolder = Path.Combine(easyReportsFolder, "العملاء(نقديه)");
