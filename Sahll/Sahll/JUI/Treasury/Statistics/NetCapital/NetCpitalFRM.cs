...
{
    public partial class NetCpitalFRM : Form
    {
        public NetCpitalFRM()
        {
            InitializeComponent();
            dgvNetCapital.Rows.Add(new object[] { "28/12/2004", 215652, 215652, 215652, 215652, 215652, 215652, 215652 });
            dgvNetCapital.Rows.Add(new object[] { "28/12/2004", 215652, 215652, 215652, 215652, 215652, 215652, 215652 });
            dgvNetCapital.Rows.Add(new object[] { "28/12/2004", 215652, 215652, 215652, 215652, 215652, 215652, 215652 });
            dgvNetCapital.Rows.Add(new object[] { "28/12/2004", 215652, 215652, 215652, 215652, 215652, 215652, 215652 });
        }
        ...
        {
            DialogFRM fRM = new DialogFRM("تم التقاط صافي رأس المال حالا", Color.FromArgb(83, 4, 128));
        ...
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvNetCapital.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                string columnName = dgvNetCapital.Columns[e.ColumnIndex].Name;
                ...
                switch (columnName)
                {
                    case "delete":
                        ...
                        break;
                    case "info":
                        NetCapitalInfoFRM FRm = new NetCapitalInfoFRM();
        ...
        private void dgvSelling_CellMouseEnter(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvNetCapital.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                {
                    case "delete":
                        dgvNetCapital.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.deleted1;
                        break;

                    case "edit":
                        dgvNetCapital.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.editd;
                        break;

                    case "info":
                        dgvNetCapital.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.infod;
                        break;
                }
            }
        }
        ...
        private void dgvSelling_CellMouseLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvNetCapital.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                {
                    case "delete":
                        dgvNetCapital.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.deletel1;
                        break;

                    case "edit":
                        dgvNetCapital.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.editl;
                        break;

                    case "info":
                        dgvNetCapital.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.infol;
                        break;
                }
            }
        }
        ...
        private void btnreport_Click(object sender, EventArgs e)
        {
            ...
            ReportToExcel(dgvNetCapital);
        }
        private void ReportToExcel(DataGridView dgv)
        {
            var title = $"تقرير بصافي رأس المال ليوم {DateTime.Now:dddd} بتاريخ {DateTime.Now:yyyy-MM-dd}";

            // تحديد مسار مجلد "تقارير سهل" داخل "سطح المكتب"
            string desktopPath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
            string easyReportsFolder = Path.Combine(desktopPath, "تقارير سهل");
            string salesReportsFolder = Path.Combine(easyReportsFolder, "صافي رأس المال");
            ...
            {
                var worksheet = workbook.Worksheets.Add("صافي رأس المال");
