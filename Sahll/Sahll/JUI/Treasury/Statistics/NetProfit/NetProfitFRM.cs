...
{
    public partial class NetProfitFRM : Form
    {
        public NetProfitFRM()
        {
            InitializeComponent();
            dgvNetProfit.Rows.Add(new object[] { "28/12/2004", 527, 2156, 2156, 2156 });
            dgvNetProfit.Rows.Add(new object[] { "28/12/2004", 524, 2156, 2156, 2156 });
            dgvNetProfit.Rows.Add(new object[] { "28/12/2004", 524, 2156, 2156, 2156 });
            dgvNetProfit.Rows.Add(new object[] { "28/12/2004", 524, 2156, 2156, 2156 });
        }
        ...
        private void btnAddSelling_Click(object sender, EventArgs e)
        {
            ...
            DialogFRM fRM = new DialogFRM("تم التقاط صافي الربح حالا", Color.FromArgb(83, 4, 128));
        ...
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvNetProfit.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                string columnName = dgvNetProfit.Columns[e.ColumnIndex].Name;
                ...
                switch (columnName)
                {
                    case "delete":
                        ...
                        DialogFRM fRM = new DialogFRM("تم حذف اللقطه", Color.FromArgb(83, 4, 128));
                        fRM.btncancel.Visible = false;
                        fRM.btnconferm.Visible = false;
                        fRM.timer1.Enabled = true;
                        fRM.Show();
                        break;
                    case "info":
                        NetProfitInfoFRM FRm = new NetProfitInfoFRM();
        ...
        private void dgvSelling_CellMouseEnter(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvNetProfit.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                {
                    case "delete":
                        dgvNetProfit.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.deleted1;
                        break;

                    case "edit":
                        dgvNetProfit.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.editd;
                        break;

                    case "info":
                        dgvNetProfit.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.infod;
                        break;
                }
            }
        }
        ...
        private void dgvSelling_CellMouseLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvNetProfit.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                {
                    case "delete":
                        dgvNetProfit.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.deletel1;
                        break;

                    case "edit":
                        dgvNetProfit.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.editl;
                        break;

                    case "info":
                        dgvNetProfit.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.infol;
                        break;
                }
            }
        }
        ...
        private void btnreport_Click(object sender, EventArgs e)
        {
            ...
            ReportToExcel(dgvNetProfit);
        }
        private void ReportToExcel(DataGridView dgv)
        {
            var title = $"تقرير بصافي الربح ليوم {DateTime.Now:dddd} بتاريخ {DateTime.Now:yyyy-MM-dd}";

            // تحديد مسار مجلد "تقارير سهل" داخل "سطح المكتب"
            string desktopPath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
            string easyReportsFolder = Path.Combine(desktopPath, "تقارير سهل");
            string salesReportsFolder = Path.Combine(easyReportsFolder, "صافي الربح");
            ...
            {
                var worksheet = workbook.Worksheets.Add("صافي الربح");
                ...
                //int totalsStartColumn = 0; // تغيير حسب عمود الإجماليات
                //int totalsEndColumn = 0; // تغيير حسب عمود الإجماليات
