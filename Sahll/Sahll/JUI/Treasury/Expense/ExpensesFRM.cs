...
namespace Sahll.JUI.Selling
{
    public partial class ExpensesFRM : Form
    {
        ...
        {
            InitializeComponent();
            dgvExpenses.Rows.Add(new object[] { "صابون سائل", 527, "خميس", " 28/12/2004" });
            dgvExpenses.Rows.Add(new object[] { " اريال", 524, "خميس", " 28/12/2004" });
            dgvExpenses.Rows.Add(new object[] { "زيت دابر املا 100 عادي", 527, "خميس", " 28/12/2004" });
            dgvExpenses.Rows.Add(new object[] { " دابر املا 100 دهبي", 524, "خميس", " 28/12/2004" });
        }
        ...
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvExpenses.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
        ...
        private void dgvSelling_CellMouseEnter(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvExpenses.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                {
                    case "delete":
                        dgvExpenses.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.deleted1;
                        break;

                    case "edit":
                        dgvExpenses.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.editd;
                        break;
                    case "returnd":
                        dgvExpenses.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.returnedd;
                        break;

                    case "info":
                        dgvExpenses.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.infod;
                        break;
                }
            }
        }
        ...
        private void dgvSelling_CellMouseLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvExpenses.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                {
                    case "delete":
                        dgvExpenses.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.deletel1;
                        break;

                    case "edit":
                        dgvExpenses.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.editl;
                        break;
                    case "returnd":
                        dgvExpenses.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.returnedl;
                        break;

                    case "info":
                        dgvExpenses.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.infol;
                        break;
                }
            }
        }
        ...
        private void btnreport_Click(object sender, EventArgs e)
        {
            ...
            ReportToExcel(dgvExpenses);
        }
        private void ReportToExcel(DataGridView dgv)
        {
            var title = $"تقرير المصروفات ليوم {DateTime.Now:dddd} بتاريخ {DateTime.Now:yyyy-MM-dd}";

            // تحديد مسار مجلد "تقارير سهل" داخل "سطح المكتب"
            string desktopPath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
            string easyReportsFolder = Path.Combine(desktopPath, "تقارير سهل");
            string salesReportsFolder = Path.Combine(easyReportsFolder, "المصروفات");
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                int totalsEndColumn = 2; // تغيير حسب عمود الإجماليات
