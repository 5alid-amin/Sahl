...
{
    public partial class ReceiptsFRM : Form
    {
        public ReceiptsFRM()
        {
            InitializeComponent();
            ...
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvReceipts.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                switch (columnName)
                {
                    case "delete":
                        ...
                        DialogFRM fRM = new DialogFRM("سيتم حذف هذا المقبوض وتسلب الاموال من الخزينه؟", Color.FromArgb(64, 0, 0));
                        fRM.Show();
                        break;
                    ...
                    case "info":
                        ...
                        ReceiptsInfo FRm = new ReceiptsInfo();
        ...
        private void btnreport_Click(object sender, EventArgs e)
        {
            ...
            ReportToExcel(dgvReceipts);
        }
        private void ReportToExcel(DataGridView dgv)
        {
            var title = $"تقرير بمالقبوضات ليوم {DateTime.Now:dddd} بتاريخ {DateTime.Now:yyyy-MM-dd}";

            // تحديد مسار مجلد "تقارير سهل" داخل "سطح المكتب"
            string desktopPath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
            string easyReportsFolder = Path.Combine(desktopPath, "تقارير سهل");
            string salesReportsFolder = Path.Combine(easyReportsFolder, "المقبوضات");
