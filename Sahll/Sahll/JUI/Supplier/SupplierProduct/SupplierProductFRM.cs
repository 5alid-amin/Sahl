...
{
    public partial class SupplierProductFRM : Form
    {
        public SupplierProductFRM()
        {
            InitializeComponent();
            dgvSupplierProduct.Rows.Add(new object[] { "صابون سائل", 527, 2156, 2156, "الخميس" });
            dgvSupplierProduct.Rows.Add(new object[] { " اريال", 524, 2156, 2156, "الخميس" });
            dgvSupplierProduct.Rows.Add(new object[] { "زيت دابر املا 100 عادي", 524, 2156, 2156, "الخميس" });
            dgvSupplierProduct.Rows.Add(new object[] { " دابر املا 100 دهبي", 524, 2156, 2156, "الخميس" });
            dgvSupplierProduct.Rows.Add(new object[] { " دابر املا 100 دهبي", 524, 2156, 2156, "الخميس" });
            dgvSupplierProduct.Rows.Add(new object[] { " دابر املا 100 دهبي", 524, 2156, 2156, "الخميس" });
            dgvSupplierProduct.Rows.Add(new object[] { " دابر املا 100 دهبي", 524, 2156, 2156, "الخميس" });
            dgvSupplierProduct.Rows.Add(new object[] { " دابر املا 100 دهبي", 524, 2156, 2156, "الخميس" });
        ...
        {
            SupplierProductAddEditFRM FRM = new SupplierProductAddEditFRM();
        ...
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvSupplierProduct.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                string columnName = dgvSupplierProduct.Columns[e.ColumnIndex].Name;

                switch (columnName)
                {
                    case "delete":

                        DialogFRM fRM = new DialogFRM("سيتم حذف هذا الصنف من من حساب المورد وكذلك من المخزن ويرد سعره الي الخزينه؟", Color.FromArgb(64, 0, 0));
                        fRM.Show();
                        break;

                    case "edit":

                        SupplierProductEditFRM FRM = new SupplierProductEditFRM();
                    ...
                    case "info":
                        ...
                        SupplierProductInfoFRM FRm = new SupplierProductInfoFRM();
        ...
        private void dgvSelling_CellMouseEnter(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvSupplierProduct.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                {
                    case "delete":
                        dgvSupplierProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.deleted1;
                        break;

                    case "edit":
                        dgvSupplierProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.editd;
                        break;
                    case "returnd":
                        dgvSupplierProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.returnedd;
                        break;

                    case "info":
                        dgvSupplierProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.infod;
                        break;
                }
            }
        }
        ...
        private void dgvSelling_CellMouseLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvSupplierProduct.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                {
                    case "delete":
                        dgvSupplierProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.deletel1;
                        break;

                    case "edit":
                        dgvSupplierProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.editl;
                        break;
                    case "returnd":
                        dgvSupplierProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.returnedl;
                        break;

                    case "info":
                        dgvSupplierProduct.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.infol;
                        break;
                }
            }
        }
        ...
        private void khButton4_MouseEnter(object sender, EventArgs e)
        {
            btnreport.Image = Properties.Resources.reportd1;
        }

        private void khButton4_MouseLeave(object sender, EventArgs e)
        {
            btnreport.Image = Properties.Resources.reportl1;
        }

        private void btnreportfun(object sender, EventArgs e)
        {
            ReportToExcel(dgvSupplierProduct);
        }
        private void ReportToExcel(DataGridView dgv)
        {
            var title = $"تقرير بحساب المورد ليوم {DateTime.Now:dddd} بتاريخ {DateTime.Now:yyyy-MM-dd}";

            // تحديد مسار مجلد "تقارير سهل" داخل "سطح المكتب"
            string desktopPath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
            string easyReportsFolder = Path.Combine(desktopPath, "تقارير سهل");
            string salesReportsFolder = Path.Combine(easyReportsFolder, "الموردين(بضاعه)");
