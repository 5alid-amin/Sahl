...
{
    public partial class SupplierCashFRM : Form
    {
        public SupplierCashFRM()
        {
            InitializeComponent();
            dgvSupplierCash.Rows.Add(new object[] { 52327, "هذا الشخص من المحتمل ان لا يرجع النقود", "الخميس", "28/12/2004", "عصام" });
            dgvSupplierCash.Rows.Add(new object[] { 52327, "هذا الشخص من المحتمل ان لا يرجع النقود", "الخميس", "28/12/2004", "عصام" });
            dgvSupplierCash.Rows.Add(new object[] { 52327, "هذا الشخص من المحتمل ان لا يرجع النقود", "الخميس", "28/12/2004", "عصام" });
        ...
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvSupplierCash.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                string columnName = dgvSupplierCash.Columns[e.ColumnIndex].Name;
                ...
                switch (columnName)
                {
                    ...
                    case "delete":
                        ...
                        DialogFRM fRM = new DialogFRM("سيتم حذف هذه النقديه من حساب المورد وترد الي الخزينه؟", Color.FromArgb(64, 0, 0));
                        fRM.Show();
                        break;
                }
            }
        ...
        private void dgvSelling_CellMouseEnter(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvSupplierCash.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                {
                    case "delete":
                        dgvSupplierCash.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.deleted1;
                        break;

                    case "edit":
                        dgvSupplierCash.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.editd;
                        break;

                    case "info":
                        dgvSupplierCash.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.infod;
                        break;
                }
            }
        }
        ...
        private void dgvSelling_CellMouseLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0 && dgvSupplierCash.Columns[e.ColumnIndex] is DataGridViewImageColumn)
            {
                ...
                {
                    case "delete":
                        dgvSupplierCash.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.deletel1;
                        break;

                    case "edit":
                        dgvSupplierCash.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.editl;
                        break;

                    case "info":
                        dgvSupplierCash.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Properties.Resources.infol;
                        break;
                }
            }
        }
        ...
        private void ReportToExcel(DataGridView dgv)
        {
            ...
            string salesReportsFolder = Path.Combine(easyReportsFolder, "الموردين(نقديه)");
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                int totalsStartColumn = 1; // تغيير حسب عمود الإجماليات
                int totalsEndColumn = 1; // تغيير حسب عمود الإجماليات
